...
document.querySelectorAll('[data-nav]').forEach(b=>b.addEventListener('click',()=>{if(b.dataset.nav==='about'){location.href='./about/?lang='+document.documentElement.lang}else if(b.dataset.nav==='competencies'){location.href='./competencies/?lang='+document.documentElement.lang}else if(b.dataset.nav==='process'){location.href='./about/?lang='+document.documentElement.lang+'#process'}else if(b.dataset.nav==='home'){if(menuOpen)toggleMenu(false);scene.scrollIntoView({behavior:reduced.matches?'instant':'smooth'})}else if(b.dataset.nav==='contact'){if(menuOpen)toggleMenu(false);document.querySelector('#home-contact').scrollIntoView({behavior:reduced.matches?'instant':'smooth'})}else{const note=document.querySelector(b.closest('.footer-nav')?'#footer-note':'#menu-note');note.hidden=false;note.textContent=document.documentElement.lang==='en'?'This page is planned for the next site phase':'Цю сторінку заплановано на наступний етап сайту'}}));
