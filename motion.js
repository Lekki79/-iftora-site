...
const words={uk:{phrase:'Від технічної задачі до готового рішення',cta:'Обговорити проєкт',menu:'Меню',replay:'Переглянути рух ще раз',back:'Повернутися до модуля',note:'Напишіть нам'},en:{phrase:'From technical challenge to complete solution',cta:'Discuss your project',menu:'Menu',replay:'Watch the motion again',back:'Return to the module',note:'Email us'}};
...
let contactOpenTimer;document.querySelector('#contact').addEventListener('click',()=>{
 ...
 const open=()=>{contactOpenTimer=null;cta.classList.remove('is-locked','is-signalling');location.href='./contacts/?lang='+document.documentElement.lang+'#request-form'};
...
function updateNavigation(){const en=document.documentElement.lang==='en',labels=en?{home:'Home',about:'About',competencies:'Competencies',process:'Our process',contact:'Contact'}:{home:'Головна',about:'Про компанію',competencies:'Компетенції',process:'Як працюємо',contact:'Контакти'};document.querySelectorAll('[data-nav]').forEach(b=>b.textContent=labels[b.dataset.nav]);document.querySelectorAll('[data-ua]').forEach(e=>e.textContent=en?e.dataset.en:e.dataset.ua);document.querySelector('#menu-note').hidden=true;document.querySelector('#footer-note').hidden=true}
document.querySelectorAll('[data-lang]').forEach(b=>b.addEventListener('click',updateNavigation));
document.querySelectorAll('[data-nav]').forEach(b=>b.addEventListener('click',()=>{if(b.dataset.nav==='about'){location.href='./about/?lang='+document.documentElement.lang}else if(b.dataset.nav==='competencies'){location.href='./competencies/?lang='+document.documentElement.lang}else if(b.dataset.nav==='process'){location.href='./process/?lang='+document.documentElement.lang}else if(b.dataset.nav==='home'){if(menuOpen)toggleMenu(false);scene.scrollIntoView({behavior:reduced.matches?'instant':'smooth'})}else if(b.dataset.nav==='contact'){location.href='./contacts/?lang='+document.documentElement.lang}else{const note=document.querySelector(b.closest('.footer-nav')?'#footer-note':'#menu-note');note.hidden=false;note.textContent=document.documentElement.lang==='en'?'This page is planned for the next site phase':'Цю сторінку заплановано на наступний етап сайту'}}));
